...
test('useAuthSession handles same-tab kakao callback landing by resolving the backend session', async () => {
  ...
  assert.match(source, /setSession\(sessionData\)/)
  assert.match(source, /setUserId\(sessionData\.user_id\)/)
  assert.doesNotMatch(source, /window\.opener\.postMessage/)
