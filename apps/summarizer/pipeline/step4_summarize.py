...
def _build_initial_prompt(article: dict) -> str:
    extra_rules = [
        ...
        "행사·세미나·협약·전시·홍보성 기사에서는 홍보 문구를 옮기지 말고 실제 결정·수치·대상·일정만 낮은 강도로 요약하세요.",
        "수치가 비정상적으로 크거나 작아 보여도 단위와 표현을 원문 그대로 유지하고 추정 환산하지 마세요.",
        "회사명·기관명보다 기사 본문이 실제로 다루는 주제를 우선해 headline/summary를 구성하세요.",
        "모바일 피드용 headline은 문장형 설명보다 핵심 명사·동사 중심으로 간결하게 쓰고, 불필요한 추정 표현 반복을 줄이세요.",
        "짧은 속보나 본문 정보가 적은 기사라도 '기사 제목·본문·원문이 전했다' 같은 메타 서술로 분량을 채우지 말고 독자에게 전달할 사건·수치·상황만 쓰세요.",
    ]
