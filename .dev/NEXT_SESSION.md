...
7. Remaining product work is operationalization: schedule the post-crawl summarizer/import runner after the 08:00 GitHub crawl artifact exists, ensure it always runs with `HOME=/Users/reddit` and explicit `DATABASE_URL`, and add a watchdog/alert on `run_report.status != success`, `failed_step`, zero usable imports, or `snapshot_generation.failed_count > 0`.
